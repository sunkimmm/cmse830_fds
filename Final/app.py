...
with tab4:
    ...
    with col1:
        ...
        st.caption("• Split suffixes: 'ti on' → 'tion', 'si on' → 'sion'")
        st.caption("• Merged words: 'documentsw ould' → 'documents would'")
        st.caption("• Hyphenation fixes via NLTK dictionary validation")
        st.caption("• Remove noise patterns, footnotes, garbled text")
    with col2:
        st.markdown("**Spell Check**")
        st.caption("• PySpellChecker for validation")
        st.caption("• Flag documents with >15% unknown words")
        st.caption("• Auto-correct common typos")
        st.caption("• Quality control across 280 projects")
    col3, col4 = st.columns(2)
    with col3:
        st.markdown("**Americanize**")
        st.caption("• British → American spelling conversion")
        st.caption("• 1,700+ word pairs loaded from dictionary")
        st.caption("• e.g., 'behaviour' → 'behavior', 'colour' → 'color'")
        st.caption("• Ensures consistency for NLP analysis")
    with col4:
        st.markdown("**N-gram Preservation**")
        st.caption("• Join multi-word terms with underscores")
        st.caption("• e.g., 'water supply' → 'water_supply'")
        st.caption("• Compound standardization via frequency analysis")
        st.caption("• Preserves semantic meaning of phrases")
